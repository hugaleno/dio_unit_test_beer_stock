...
public class BeerServiceTest {
  ...
  void whenNotRegisteredBeerNameIsGivenThenThrownAnException() {
    // given
    BeerDTO notFoundBeerDTO = BeerDTOBuilder.builder().build().toBeerDTO();

    // when
    when(beerRepository.findByName(notFoundBeerDTO.getName())).thenReturn(Optional.empty());

    // then
    assertThrows(BeerNotFoundException.class,
        () -> beerService.findByName(notFoundBeerDTO.getName()));
  }
  ...
  void whenListBeerIsCalledThenReturnAListOfBeers() {
    ...
    assertThat(foundBeerList.get(0), is(equalTo(expectedFoundBeerDTO)));
  }
  ...
  void whenIncrementIsCalledThenIncrementBeerStock()
    ...
    // then
    BeerDTO incrementedBeerDTO = beerService
  ...
  void whenDecrementIsCalledThenDecrementBeerStock() throws BeerNotFoundException {
    ...
    BeerDTO decrementedBeerDTO = beerService
  ...
  @Test
  void whenDecrementIsLowerThanZeroThenQuantityShouldBeZero() throws BeerNotFoundException {
    BeerDTO expectedBeerDTO = BeerDTOBuilder.builder().build().toBeerDTO();
    Beer expectedBeer = beerMapper.toModel(expectedBeerDTO);
    ...
    int quantityAfterDecrement = 0;
    BeerDTO decrementedBeerDTO = beerService
        .decrement(expectedBeerDTO.getId(), quantityToDecrement);

    assertThat(quantityAfterDecrement, equalTo(0));
    assertThat(quantityAfterDecrement, equalTo(decrementedBeerDTO.getQuantity()));
